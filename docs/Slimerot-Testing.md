# Slimerot validation — prompts 1–15

Current Prompt 16 validation: **2,158 full headless checks**, **215 focused rendered final checks**, **68 rendered mobile UI checks**, **106 rendered combat checks**, **261 external process-kill/reopen checks**, and **223 isolated export checks**, all passing. Packed main scene completed 120 frames. The 164 migration checks are included in the full/focused suites; counts overlap. See the [final report](Slimerot-Prompt-16.md), [204-row requirement matrix](Slimerot-Prompt-16-Matrix.md), [balance/benchmarks](Slimerot-Prompt-16-Balance.md), [process-death matrix](Slimerot-Prompt-16-Saves.md) and [provisional values](Slimerot-Prompt-16-Provisional.md). Physical Android/airplane-mode and human campaign acceptance remain **NOT RUN**.

Final focus: `godot --headless --path <project> -- --slimerot-test --slimerot-final-only`. Add `--slimerot-capture` and omit `--headless` for the normal/compact record-summary captures. The final stress drives 1,800 real commits at 0.50-second ticks, and 1,000 Equip Best calls on 192 million compact identities. The standalone estimator defers until 54 live definitions are ready and checks central luck before producing reports. Early Prompt16 estimates before that guard are invalid and excluded from the final evidence.

All following stage-specific results are historical snapshots; current designs and balance are in the Prompt16 documents. Historical UI names, pacing targets and numeric tables are not current requirements.
...
Historical Prompt 15 validation: **1,943 full headless checks**, **106 focused rendered combat checks**, and **223 isolated exported-resource checks**, all passing. Packed main scene completed 120 frames. The focused suite runs with `--slimerot-test --slimerot-combat-presentation-only`; add `--slimerot-capture` without `--headless` for boss and reveal captures. It exercises real touch Dash, physics-wall collision, active-only i-frames, 1,000 pooled projectile lifetimes, normal friendly fire, exact beam geometry, animated teleport stages, saved combined gates and one-time zone luck presentation. See the [Prompt 15 report](Slimerot-Prompt-15.md). Physical Android hardware and human difficulty/playfeel remain unverified.

Historical Prompt 14 validation: **1,837 full headless checks**, **68 focused rendered UI checks** and **178 isolated export checks**, all passing, plus a successful 120-frame packed main-scene launch. The final full-suite count is recorded in the [Prompt 14 report](Slimerot-Prompt-14.md). Captures cover 720×1280, 720×1560 and 800×1280. Android touch-event pan/pinch, real touch purchases, one-layer Back, 20-cycle closes, clipped hit testing, app-suspension cleanup, preserved pan and closed-Team Auto Roll node counts are covered. Physical Android hardware remains untested.
...
34. On physical Android hardware, complete a long offline session with simultaneous movement/Auto Roll, menu use, boss dodging, pause/background and OS-kill recovery. APK export, battery/performance, touch/audio and a representative campaign remain outstanding acceptance work; the old 3–3.5-hour target is superseded by Prompt16.
