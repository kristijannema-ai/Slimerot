...
Current Prompt 15 validation: **1,943 full headless checks**, **106 focused rendered combat checks**, and **223 isolated exported-resource checks**, all passing. Packed main scene completed 120 frames. The focused suite runs with `--slimerot-test --slimerot-combat-presentation-only`; add `--slimerot-capture` without `--headless` for boss and reveal captures. It exercises real touch Dash, physics-wall collision, active-only i-frames, 1,000 pooled projectile lifetimes, normal friendly fire, exact beam geometry, animated teleport stages, saved combined gates and one-time zone luck presentation. See the [Prompt 15 report](Slimerot-Prompt-15.md). Physical Android hardware and human difficulty/playfeel remain unverified.
