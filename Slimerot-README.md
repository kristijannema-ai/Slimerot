...
All 24 bases are rollable from the start; zone metadata identifies origin only. Effective Luck multiplies tree/Breakthrough/Coin effects, unlocked gameplay zones (Z1 ×1 through Z8 ×8, excluding Hub), potion and one-roll Super. The score is L/U with U in (0,1], selecting the highest threshold reached, otherwise Tung Tung. Zone luck is derived on every read and never compounded on load.
