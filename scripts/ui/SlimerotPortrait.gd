...
static func paint(surface: CanvasItem, center: Vector2, radius: float, id: String, kind: String, hidden: bool, time: float, squash: float = 0.0) -> void:
	var flags := SlimerotVariants.mask(kind)
	var tone := Color.from_hsv(float(absi(id.hash()) % 360) / 360.0, 0.34, 0.91)
	if (flags & SlimerotVariants.GOLDEN) != 0: tone = Color("ffcf63")
	...
	if sprite != null:
		...
		if not hidden and (flags & SlimerotVariants.GOLDEN) != 0:
			tint = Color(1.0, 0.82, 0.4)
			surface.draw_circle(at, radius * 1.38, Color(1, 0.8, 0.2, 0.18))
		if not hidden and (flags & SlimerotVariants.GLITCHED) != 0:
		...
		if not hidden and (flags & SlimerotVariants.SHINY) != 0:
			surface.draw_arc(at, radius * (1.30 + sin(time * 5.0) * 0.06), 0, TAU, 32, Color("bcfaff"), 2, true)
			if not (flags & SlimerotVariants.GOLDEN): tint = Color(0.86, 1.0, 1.0)
		if not hidden and (flags & SlimerotVariants.GOLDEN):
			for ray in 8:
				var axis := Vector2.from_angle(time * 0.4 + ray * TAU / 8.0)
				surface.draw_line(at + axis * radius, at + axis * radius * 1.7, Color(1, 0.82, 0.25, 0.38), 3.0, true)
		...
		elif (flags & SlimerotVariants.SHINY) != 0:
			for index in 4:
	...
	if (flags & SlimerotVariants.GLITCHED) != 0 and not hidden:
	...
	if (flags & SlimerotVariants.GOLDEN) != 0 and not hidden:
	...
	if hidden:
		...
		return
	if (flags & SlimerotVariants.SHINY) != 0:
