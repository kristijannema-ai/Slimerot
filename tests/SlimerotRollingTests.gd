...
func run(world: Node, owner_suite: Node) -> void:
	...
	check(not SkillTreeManager.purchase("C06"), "slot prerequisites require Slime Bond II")
	SkillTreeManager.purchase("C01")
	check(SkillTreeManager.purchase("C02") and SkillTreeManager.derived_stats().equipped_slots == 2, "Slot 2 unlocks for 350 Coins")
	check(not SkillTreeManager.purchase("C06"), "Slot 3 requires Z2 boss")
	for zone_id in [2,4,6]: GameState.boss_defeated_flags["zone_%d" % zone_id] = true
	for id in ["C05", "C06", "C08", "C10", "C13", "C15"]: SkillTreeManager.purchase(id)
