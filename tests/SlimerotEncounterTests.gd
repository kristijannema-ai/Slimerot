...
func run(world: Node, owner_suite: Node) -> void:
	...
	for index in 4:
		...
		if zone == 2:
			...
			boss.step(0.02)
			check(GameState.player_hp == 76 and boss.phase == "slam_release", "Espresso warning resolves at 0.8s for exactly 24 damage before its second warning")
		elif zone == 4:
			boss.step(3.2)
			check(boss.phase == "fan_warning" and CombatManager.active_projectile_count() == 0, "Sand Router warns before its opening four-second fan")
			boss.step(0.8)
			check(CombatManager.active_projectile_count() == 5 and CombatManager.active_projectiles()[0].damage == 50, "Sand Router opens with five 50-damage shots before its beam sequence")
		elif zone == 6:
			boss.step(2.2)
			check(boss.phase == "teleport_fadeout" and boss.teleport_index == 0, "Janitor begins a visible fade before changing position")
			boss.step(SlimerotBoss.FADE_OUT_SECONDS)
			boss.step(SlimerotBoss.TELEPORT_MARKER_SECONDS)
			check(boss.teleport_index == 1 and boss.position == SlimerotEncounters.TELEPORT_POINTS[0] and boss.phase == "teleport_fadein", "Janitor arrives at the warned arena point and fades in")
			boss.step(SlimerotBoss.FADE_IN_SECONDS)
			boss.step(SlimerotBoss.BURST_WARNING_SECONDS)
			boss.step(0.2)
			boss.step(SlimerotBoss.BURST_WARNING_SECONDS)
			check(CombatManager.active_projectile_count() == 6 and CombatManager.active_projectiles()[0].damage == 80, "Janitor fires two separately warned three-shot bursts for 80 damage")
		else:
			boss.step(3.2)
			boss.step(0.8)
			boss.step(0.3)
			check(CombatManager.active_projectile_count() == 5 and CombatManager.active_projectiles()[0].damage == 140 and boss.admin_teleport, "Admin advances from five 140-damage fan shots into its teleport sequence")
