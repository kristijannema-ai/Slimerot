...
func run(world: Node, owner_suite: Node) -> void:
	...
	var shots := CombatManager.active_projectile_count()
	CombatManager._physics_process(0.99)
	check(shots == 2 and CombatManager.active_projectile_count() == 2, "two equipped slimes independently fire once, then wait a full second")
	CombatManager._physics_process(0.02)
	check(CombatManager.active_projectile_count() == 4, "both independent attack timers fire again after one second")
	CombatManager.clear_projectiles()
